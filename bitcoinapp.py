...
def developer_guides():
    webbrowser.open('https://developer.bitcoin.org/devguide/index.html',new=0,autoraise=True)
...
label1=tk.Label(text=f'直近のブロック生成時間= {minutes_between}',foreground='orange',background='black',font=("MSゴシック","20","bold"))
label2=tk.Label(text=f'ブロックの高さ= {height}',foreground='orange',background='black',font=("MSゴシック","20","bold"))
label3=tk.Label(text=f'前回の半減期ブロック数= {last_halflife}',foreground='orange',background='black',font=("MSゴシック","20","bold"))
label4=tk.Label(text=f'次の半減期までの残りブロック数= {last_halflife+210000-height}',foreground='orange',background='black',font=("MSゴシック","20","bold"))
label5=tk.Label(text=f'次の半減期予想時間= {pre.year}年{pre.month}月{pre.month}日{pre.month}時{pre.minute}分',foreground='orange',background='black',font=("MSゴシック","20","bold"))
label6=tk.Label(text=f'現在時刻：{datetime.datetime.now()}',foreground='orange',background='black',font=('MSゴシック',22,'bold'))
...
canvas = FigureCanvasTkAgg(fig2, master=root)  # Generate canvas instance, Embedding fig in root
canvas.draw()
canvas.get_tk_widget().grid(row=0,column=1,sticky=tk.NE)
button1 = tk.Button(root,text=f'ハッシュレート= {hash_rate}',font=('MSゴシック',20,'bold'),command=chart3)
button2 = tk.Button(root,text=f'採掘難易度(Difficulty)= {difficulty}',font=('MSゴシック',20,'bold'),command=chart2)
button3 = tk.Button(root,text=f'BTC/USD＝ {market_price_usd}',font=('MSゴシック',20,'bold'),command=chart1)
